"""IOSRNodeInterface — Reticulum custom interface for RNode LoRa hardware on iOS.

Port of Columba Android's `IOSRNodeInterface`. The KISS framing, frame
parsing, and RNode binary protocol are reused verbatim (proven on Android, in
turn ported from upstream `RNS.Interfaces.RNodeInterface`). Only the I/O layer
differs: instead of the Kotlin BLE bridge (jnius), we bridge to the Swift
`SwiftRNodeBridge` CoreBluetooth Nordic-UART client via ctypes `columba_rnode_*`
C-ABI shims, with Swift→Python notifications delivered through the `rns_bridge`
callback registry — exactly the pattern `IOSBLEInterface` / `IOSBLEDriver` use
for the BLE mesh.

**BLE (Nordic UART Service) only** — no USB-serial, no Bluetooth Classic (no
iOS support). RNode flashing is out of scope (assume a pre-flashed RNode).

RNS external-interface loader contract:
    • file:   <configdir>/interfaces/IOSRNodeInterface.py
    • config: type = IOSRNodeInterface
...
# Make sibling modules + rns_bridge importable when Reticulum exec()s this file
# from <configdir>/interfaces/ (mirrors IOSBLEInterface).
_this_file = globals().get("__file__")
_interfaces_dir = os.path.dirname(os.path.abspath(_this_file)) if _this_file else None
if _interfaces_dir and _interfaces_dir not in sys.path:
    sys.path.insert(0, _interfaces_dir)

import rns_bridge  # Swift→Python callback registry (set_rnode_callback)

# ── ctypes binding to SwiftRNodeBridge's C-ABI shims (Python → Swift) ──
try:
    _lib = ctypes.CDLL(None)  # symbols are statically linked into the app binary
except OSError:
    _lib = None
...
_rnode_start = _decl("columba_rnode_start", [], ctypes.c_int32)
_rnode_stop = _decl("columba_rnode_stop", [], ctypes.c_int32)
_rnode_connect = _decl("columba_rnode_connect", [ctypes.c_char_p], ctypes.c_int32)
_rnode_disconnect = _decl("columba_rnode_disconnect", [], ctypes.c_int32)
_rnode_write = _decl("columba_rnode_write", [ctypes.c_char_p, ctypes.c_int32], ctypes.c_int32)


class _RNodeBLEBridge:
    """Presents the `KotlinRNodeBridge` method surface the ported protocol code
    expects (connect / isConnected / getConnectedDeviceName / setOnDataReceived
    / setOnConnectionStateChanged / writeSync / disconnect) over the Swift
    `SwiftRNodeBridge` C-ABI shims. One per process.

    - Python → Swift: ctypes `columba_rnode_*` (shims return 0 on success).
    - Swift → Python: `rns_bridge.set_rnode_callback("data"|"state", cb)` — Swift
      invokes the registered callbacks (via PythonRNodeCallbackBridge, Python.h)
      on NUS TX notify / connection-state change.
    """
    ...
    def __init__(self) -> None:
        ...
        # Inbound NUS TX bytes are PUSHED from Swift (didUpdateValueFor) but the
        # ported protocol drains them by POLLING `read()` in `_read_loop`. Bridge
        # the two models with a thread-safe buffer: `_raw_on_data` appends here,
        # `read()` drains. This is exactly how Android's KotlinRNodeBridge behaves
        # (its registered on_data callback is a no-op — see _on_data_received).
        self._rx_buffer = bytearray()
        self._rx_lock = threading.Lock()
        rns_bridge.set_rnode_callback("data", self._raw_on_data)
        rns_bridge.set_rnode_callback("state", self._raw_on_state)
        if _rnode_start:
            _rnode_start()

    def connect(self, device_name: str, mode: Any = None) -> bool:
        if _rnode_connect is None or not device_name:
            return False
        self._device_name = device_name
        return _rnode_connect(device_name.encode("utf-8")) == 0

    def isConnected(self) -> bool:
        return self._connected

    def getConnectedDeviceName(self) -> Optional[str]:
        return self._device_name if self._connected else None

    def setOnDataReceived(self, cb: Callable) -> None:
        self._on_data = cb

    def setOnConnectionStateChanged(self, cb: Callable) -> None:
        self._on_state = cb

    def writeSync(self, data: bytes) -> int:
        """Write to the RNode RX characteristic. Returns bytes accepted
        (== len on success) to satisfy the protocol's `written == len(data)`."""
        if _rnode_write is None:
            return 0
        b = bytes(data)
        return len(b) if _rnode_write(b, len(b)) == 0 else 0

    def disconnect(self) -> None:
        if _rnode_disconnect:
            _rnode_disconnect()
        self._connected = False

    def read(self) -> bytes:
        """Drain buffered inbound serial (the poll side of `_read_loop`).
        Non-blocking — returns b"" when empty so the read loop sleeps 10ms
    ...
    def notifyOnlineStatusChanged(self, is_online: bool, name: str) -> None:
        """No-op on iOS. On Android this drives a system heads-up notification
        ("RNode Disconnected") via a bridge listener; iOS surfaces interface
        state through the in-app NetworkStatus UI instead. Present so the
        ported `_set_online` path doesn't take its try/except fallback."""
        return None

    # ── Swift → Python callbacks (invoked from the rns_bridge registry) ──
    def _raw_on_data(self, data: bytes) -> None:
        # Buffer for `read()` — this IS the inbound data path (the protocol's
        # own on_data handler is a no-op; bytes are consumed by polling).
        with self._rx_lock:
            self._rx_buffer.extend(bytes(data))

    def _raw_on_state(self, connected: bool, device_name: Optional[str] = None) -> None:
        self._connected = bool(connected)
        if device_name:
            self._device_name = device_name
        if self._on_state is not None:
            try:
                self._on_state(bool(connected), self._device_name)
            except Exception as e:  # noqa: BLE001
                RNS.log(f"IOSRNodeInterface: on_state callback raised: {e}", RNS.LOG_ERROR)
...
class IOSRNodeInterface(Interface):
    """
    ...
    def __init__(self, owner, configuration):
        ...
        self.HW_MTU = IOSRNodeInterface.HW_MTU
        ...
        if mode_str == "full":
            ...
            RNS.log(f"IOSRNodeInterface '{self.name}': unknown mode '{mode_str}', defaulting to full", RNS.LOG_WARNING)
            self.mode = Interface.MODE_FULL

        # Connection target + mode. iOS supports BLE (Nordic UART Service)
        # ONLY — no USB-serial, no Bluetooth Classic. Force BLE regardless of
        # what the config says so a stale/other mode can't reach the USB/Classic
        # code paths (dead on iOS).
        self.connection_mode = self.MODE_BLE
        ...
        if self.connection_mode == "tcp":
            RNS.log(
                f"IOSRNodeInterface '{self.name}': connection_mode='tcp' "
        ...
        try:
            ...
            RNS.log(
                f"IOSRNodeInterface '{self.name}': invalid config — {e}",
                RNS.LOG_ERROR,
            )
            return

        RNS.log(f"IOSRNodeInterface '{self.name}' initialized", RNS.LOG_DEBUG)
    ...
    def _safe_start(self):
        ...
        try:
            ...
            RNS.log(
                f"IOSRNodeInterface[{self.name}] start() raised: {e} — interface staying offline",
                RNS.LOG_ERROR,
            )
            import traceback
            traceback.print_exc()
    ...
    def _get_kotlin_bridge(self):
        """Instantiate the iOS BLE bridge (Swift SwiftRNodeBridge over ctypes).

        Replaces Android's jnius/event_bridge resolution. `_RNodeBLEBridge`
        presents the same method surface the protocol code expects, so the rest
        of this file is unchanged from the Android port.
        """
        try:
            self.kotlin_bridge = _RNodeBLEBridge()
            RNS.log("IOSRNodeInterface: SwiftRNodeBridge (ctypes) initialised", RNS.LOG_DEBUG)
        except Exception as e:  # noqa: BLE001
            self.kotlin_bridge = None
            RNS.log(f"IOSRNodeInterface: failed to init SwiftRNodeBridge: {e}", RNS.LOG_ERROR)
    ...
    def _get_usb_bridge(self):
        ...
        try:
            ...
            if self.usb_bridge is not None:
                RNS.log("IOSRNodeInterface: KotlinUSBBridge resolved via usb_bridge module", RNS.LOG_DEBUG)
            else:
                RNS.log(
                    "IOSRNodeInterface: KotlinUSBBridge not available "
                    "(usb_bridge.get_usb_bridge() returned None) — USB mode will not function",
                    RNS.LOG_ERROR,
                )
        except Exception as e:  # noqa: BLE001
            RNS.log(f"IOSRNodeInterface: failed to get KotlinUSBBridge: {e}", RNS.LOG_ERROR)
    ...
    def start(self):
        ...
        try:
            ...
            RNS.log(f"IOSRNodeInterface: optional callback registration failed (non-fatal): {e}", RNS.LOG_DEBUG)
        ...
        if self._read_thread is not None and self._read_thread.is_alive():
            RNS.log(
                f"IOSRNodeInterface[{self.name}]: stopping stale BLE/Classic "
            ...
            if self._read_thread.is_alive():
                RNS.log(
                    f"IOSRNodeInterface[{self.name}]: stale read thread did not stop "
    ...
    def _start_usb(self):
        ...
        try:
            ...
            RNS.log(f"IOSRNodeInterface: optional USB callback registration failed (non-fatal): {e}", RNS.LOG_DEBUG)
    ...
    def __str__(self):
        return f"IOSRNodeInterface[{self.name}]"
...
interface_class = IOSRNodeInterface
