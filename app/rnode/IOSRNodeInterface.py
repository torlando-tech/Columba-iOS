"""Columba's iOS RNS interface for BLE-connected RNode LoRa hardware.

The KISS framing, frame parsing and RNode protocol are ported from the proven
Android Columba interface. Only the byte-stream transport differs: iOS routes
Nordic UART Service bytes through ``IOSRNodeDriver`` into ReticulumSwift's
native ``BLETransport`` via C-ABI shims. CoreBluetooth remains Swift-owned;
Python remains the authoritative RNS/KISS owner.

External-interface loader contract (Reticulum.py:919-937):
    • file path:   <configdir>/interfaces/IOSRNodeInterface.py
    • config type: type = IOSRNodeInterface
    • module footer: interface_class = IOSRNodeInterface
...
class IOSRNodeInterface(Interface):
    ...
    over Bluetooth Low Energy (Nordic UART Service). The byte stream is
    provided by ``IOSRNodeDriver`` and the native Swift transport bridge.
    """
    ...
    def __init__(self, owner, configuration):
        ...
        # The shipping iOS bridge is BLE-only. Refuse stale Android/desktop
        # modes truthfully instead of leaving a registered-but-dead interface.
        if self.connection_mode != self.MODE_BLE:
            RNS.log(
                f"IOSRNodeInterface '{self.name}': connection_mode="
                f"'{self.connection_mode}' is unsupported on iOS; use 'ble'.",
                RNS.LOG_ERROR,
            )
            return
    ...
    def _get_kotlin_bridge(self):
        """Construct the native iOS BLE byte-stream adapter."""
        try:
            from IOSRNodeDriver import IOSRNodeDriver
            self.kotlin_bridge = IOSRNodeDriver()
            RNS.log("IOSRNodeInterface: native Swift BLE bridge ready", RNS.LOG_DEBUG)
        except Exception as e:  # noqa: BLE001
            self.kotlin_bridge = None
            RNS.log(f"IOSRNodeInterface: native Swift BLE bridge unavailable: {e}", RNS.LOG_ERROR)
