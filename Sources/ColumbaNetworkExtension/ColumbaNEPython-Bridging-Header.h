...
// bridging path as the app (the hard collision rule in NEReticulumNode.swift:
// a file is either a ReticulumSwift file or a Python file, never both). This
// header exposes ONLY the CPython C API so the NE's embedded-Python runtime
// (NEPythonRuntime) can call Py_* directly without dragging in RNSAPI's
// re-declared RNS types.
//
